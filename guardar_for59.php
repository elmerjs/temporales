...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Nota: El punto 5 (calificaciones) viene en HTML desde el editor, por lo que los 
    // puntajes calculados y digitados por el usuario viajan íntegros en esta variable.
    ...
    // =================================================================================
    // 7. LÓGICA CORREGIDA: VERIFICAR SI YA EXISTE PARA HACER UPDATE O INSERT
    // =================================================================================
    
    // Primero, verificamos si ya existe el acta para este departamento y semestre
    $sql_check = "SELECT id_acta FROM actas_seleccion_docente WHERE departamento_id = ? AND anio_semestre = ?";
    $stmt_check = $conn->prepare($sql_check);
    $stmt_check->bind_param("ss", $departamento_id, $anio_semestre);
    $stmt_check->execute();
    ...
    if ($fila = $resultado_check->fetch_assoc()) {
        // EXISTE EL ACTA -> HACEMOS UPDATE
        $id_acta_existente = $fila['id_acta'];
        
        $sql_update = "UPDATE actas_seleccion_docente SET 
        ...
        $ejecucion_exitosa = $stmt_update->execute();
        $stmt_update->close();

    } else {
        // NO EXISTE -> HACEMOS INSERT
        $sql_insert = "INSERT INTO actas_seleccion_docente (
        ...
        $ejecucion_exitosa = $stmt_insert->execute();
        $stmt_insert->close();
    }
    ...
    // =================================================================================
    // 8. LÓGICA DE REDIRECCIÓN Y NOTIFICACIÓN
    // =================================================================================
